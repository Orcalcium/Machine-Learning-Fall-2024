...
for _ in range(num_experiments):
    ...
    Eout_lin = squared_error(X_test, y_test, w_lin)
    Eout_poly = squared_error(X_poly_test, y_poly_test, w_poly)

    # Calculate the difference in Ein and store in Esqr_diff_list
    Ein_diff = Eout_lin - Eout_poly
    Ein_diff_list.append(Ein_diff)

# Calculate the average Ein gain
average_Ein_gain = np.mean(Ein_diff_list)
print("Average E_out_lin - E_out_poly (E_out gain):", average_Ein_gain)

# Plot a histogram of Esqr_diff_list
plt.figure(figsize=(10, 5))
plt.hist(Ein_diff_list, bins=200, edgecolor='black')
plt.title("Histogram of E_out_lin - E_out_poly")
plt.xlabel("E_out_lin - E_out_poly")
...
# Plot a histogram of Esqr_diff_list
plt.figure(figsize=(10, 5))
plt.hist(Ein_diff_list, bins=100, range=(-1e8, max(Ein_diff_list)), edgecolor='black')
plt.title("Histogram of E_out_lin - E_out_poly")
plt.xlabel("E_out_lin - E_out_poly")
plt.ylabel("Frequency")
plt.grid(True)
